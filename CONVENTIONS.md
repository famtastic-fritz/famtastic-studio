...
8. Paths registry: `config/paths.json` with roots `sites, apps, previews, media, components, dna, recipes, journal, conversations, events, execution`. All relative to `STUDIO_DATA_ROOT` (default `../.studio-next-data`, gitignored). Only `kernel/paths.js` joins paths.
