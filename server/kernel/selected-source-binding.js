...
import { creditSourceProjection, matchesSourceArtifact, sourcePathsMatch } from './selected-credit-source.js';
...
export function createSelectedSourceResolver({ paths, mappings = [], getMappings = () => [] }) {
  return async (packet, { reconcile = false } = {}) => {
    ...
    const credit = creditSourceProjection(record);
    ...
    if (ancestor) {
      ...
      if (old.sha256 !== ancestor.source_export_sha256 || old.run_id !== ancestor.run_id || (!old.scope_complete && !verifiedPartialAssociation(old, m)) || old.site_id !== record.site_id || old.repository.repository_path !== record.repository.repository_path
        ...
        || !sourcePathsMatch(pathsInPacket, old, creditSourceProjection(old))) throw stagingError('source_repository_ancestor_mismatch');
      ...
      for (const file of old.files) {
        const priorCredit = creditSourceProjection(old);
        const declared = c.files.find(f => f.path === file.path), artifact = packet.artifacts.find(a => a.path === declared?.source_path);
        if (!declared && priorCredit?.added.some(f => f.path === file.path)) continue;
        if (!matchesSourceArtifact(file, artifact, priorCredit)) throw stagingError('source_repository_ancestor_bytes_changed');
      }
    }
    if (!reconcile && !sourcePathsMatch(pathsInPacket, record, credit)) throw stagingError('source_export_manifest_mismatch');
    ...
    for (const file of record.files) {
      ...
      const internalAddition = !declared && credit?.added.some(f => f.path === file.path);
      if (bytes.length !== file.bytes || digest(bytes) !== file.sha256
        || !internalAddition && (declared || !reconcile) && !matchesSourceArtifact(file, artifact, credit)) throw stagingError('source_export_bytes_changed');
    }
    return { outcome: 'success', site_id: record.site_id, run_id: record.run_id,
      ...
      source_export: wire, reused_existing_source: true, source_mapping_ref: m.evidence_ref, mapping: m, credit_projection: credit };
  };
}
