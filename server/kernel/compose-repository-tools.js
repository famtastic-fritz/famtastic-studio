...
export function publicFiles() {
  ...
  if (!Array.isArray(listed)) throw new Error('Invalid public file allowlist');
...
const receiptFile = path.resolve(root, git(['rev-parse', '--git-path', 'famtastic-static-dist.json']));
function inventory(dir) {
  const entries = [];
  function walk(rel = '') {
    const absolute = path.join(dir, rel); const stat = fs.lstatSync(absolute);
    if (stat.isSymbolicLink()) throw new Error('Generated output may not contain symlinks');
    if (stat.isDirectory()) {
      if (rel) entries.push({ path: rel, type: 'directory' });
      for (const name of fs.readdirSync(absolute).sort()) walk(rel ? rel + '/' + name : name);
    } else if (stat.isFile()) entries.push({ path: rel, type: 'file', sha256: crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex') });
    else throw new Error('Unsupported generated output entry');
  }
  walk(); return entries;
}
const hasPrior = fs.existsSync(destination);
if (hasPrior) {
  if (fs.lstatSync(destination).isSymbolicLink() || !fs.statSync(destination).isDirectory()) throw new Error('dist must be a real local output directory');
  if (!fs.existsSync(receiptFile) || fs.lstatSync(receiptFile).isSymbolicLink()) throw new Error('Unowned dist must not be replaced');
  const prior = JSON.parse(fs.readFileSync(receiptFile, 'utf8'));
  if (prior.schema_version !== 1 || prior.output !== 'dist' || JSON.stringify(prior.entries) !== JSON.stringify(inventory(destination))) throw new Error('Authored or changed dist must not be replaced');
}
const staging = fs.mkdtempSync(path.join(root, '.site-build-'));
const backup = path.join(root, '.site-build-previous-' + crypto.randomUUID());
const nextReceipt = receiptFile + '.' + crypto.randomUUID();
let previousMoved = false; let installed = false;
try {
  ...
  fs.writeFileSync(nextReceipt, JSON.stringify({ schema_version: 1, output: 'dist', entries: inventory(staging) }), { flag: 'wx' });
  if (hasPrior) { fs.renameSync(destination, backup); previousMoved = true; }
  fs.renameSync(staging, destination); installed = true;
  fs.renameSync(nextReceipt, receiptFile);
  console.log('Built ' + files.length + ' allowlisted public files into dist/');
} catch (error) {
  if (installed) fs.rmSync(destination, { recursive: true });
  if (previousMoved) { fs.renameSync(backup, destination); previousMoved = false; }
  throw error;
} finally {
  if (fs.existsSync(staging)) fs.rmSync(staging, { recursive: true });
  if (fs.existsSync(nextReceipt)) fs.unlinkSync(nextReceipt);
}
if (previousMoved) fs.rmSync(backup, { recursive: true }); // Only exact receipted generated bytes.
`;
