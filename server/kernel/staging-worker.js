...
import { deriveSelectedReviewArtifact, assertSelectedReviewRights } from './selected-review-artifact.js';
...
export async function materializeSelection(packet, { fetchArtifact, allowedArtifactOrigins, readMappedArtifact, resolveCompleted }) {
  ...
  const artifactBundle = createArtifactBundle(files);
  assertSelectedReviewRights(packet, deriveSelectedReviewArtifact(artifactBundle).files);
  const prepared = prepareStagingBuildPacket({
    ...
    spec: c.spec, brand: c.brand, artifact_bundle: artifactBundle, research_packet_ref: c.research_packet_ref,
