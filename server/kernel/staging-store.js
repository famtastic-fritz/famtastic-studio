...
export function createStagingStore({ paths, journal }) {
  ...
  async function readMappedArtifact(packet, file, options) {
    ...
    const original = resolved.credit_projection?.originals.find(entry => entry.path === file.path);
    const declared = packet.artifacts.find(entry => entry.path === file.source_path);
    if (original && original.sha256 !== source.sha256 && original.sha256 === declared?.sha256 && original.bytes === declared?.bytes) {
      return Buffer.from(original.content_base64, 'base64');
    }
