...
it('signed HTTP acceptance wakes the actual worker and persists one end-to-end job', async () => {
  ...
  req.method = 'POST'; req.url = '/api/pipeline/selected-staging/accept'; req.headers = { 'x-famtastic-signature': `sha256=${crypto.createHmac('sha256', 'synthetic-ingress').update(raw).digest('hex')}` };
  ...
  const done = f.store.read(response.body.receipt.receipt_id);
  expect(done.state, JSON.stringify({ failure: done.failure, qa: done.qa, build: done.build?.error })).toBe('complete');
