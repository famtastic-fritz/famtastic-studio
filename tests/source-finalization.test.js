...
it('exports real committed pipeline output without equating pipeline success with completed scope', async () => {
  ...
  const transform = initial.provenance.creator_credit_transform;
  expect(transform.original_files[0].sha256).toBe(packet().artifacts[0].sha256);
  expect(initial.files.find(file => file.path === 'index.html').sha256).toBe(transform.changed_pages[0].after_sha256);
  expect(initial.files.some(file => file.path.startsWith('.famtastic/'))).toBe(false);
