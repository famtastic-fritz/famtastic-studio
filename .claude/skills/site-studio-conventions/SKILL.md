...
## Non-negotiable
0. **Independent customer source.** Read AGENTS.md and docs/capabilities/SITE-REPOSITORY-STANDARD.md. Use the single vendor/site-foundation package before site writes; preserve authored docs/backend source, fail on wrong or dirty targets and never infer a remote URL. Libraries are independent pinned catalogs, not executable sibling-folder imports. Business owner and builder are distinct.
